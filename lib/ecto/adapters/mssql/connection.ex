if Code.ensure_loaded?(Mssqlex) do
  defmodule Ecto.Adapters.MSSQL.Connection do
    ...
    def execute_ddl({command, %Table{} = table, columns}) when command in @creates do
      ...
      query = [if_do(command == :create_if_not_exists, "IF NOT EXISTS (select * from sysobjects where name='#{table_name}' and xtype='U') "),
             "CREATE TABLE ",
               table_name, ?\s, ?(,
    ...
    def ddl_logs(%Mssqlex.Result{} = result) do
      ...
      # the driver during the execution of the query
			# There is no messages in ODBC adapter
      []
    end
    ...
    defp column_type(type, opts) do
      ...
      cond do
        size            -> [add_size(type_name, size)]
        precision       -> [type_name, ?(, to_string(precision), ?,, to_string(scale || 0), ?)]
        type == :string -> [add_size(type_name, 255)]
        true            -> type_name
      end
    end

		defp add_size(type_name, size),
			do: String.replace(type_name, ~r/\(\d+\)/, "(#{size})")
    ...
    def ecto_to_db(:string), do: "varchar(8000)"
